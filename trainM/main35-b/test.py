...
class Tester():
    ...
    def evaluate(self,lr):
        ...
        lr = lr.permute((2,0,1)).unsqueeze(0) / 255.0
        sr = self.SRmodels[-1](lr)
        sr = sr.clamp(0,1).squeeze(0).permute(1,2,0).data.cpu().numpy() * 255
        return sr
    ...
    # test basic model
    def testbasic(self,save=False,quick=False):
        scores = {}

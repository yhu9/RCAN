...
class SISR():
    ...
    def optimize(self,data,iou_threshold=0.7,miniter=500):
        ...
        while True:
            ...
            selectionloss = -1 * torch.mean(probs.gather(1, minidx.unsqueeze(1)).clamp(1e-16).log())
            sisrloss2 = torch.abs(SR_result - hrbatch)
            #sisrloss = sisrloss.unsqueeze(1)
            #sisrloss = minval.mean()
            sisrloss = sisrloss.mean()
            if self.model == 'RCAN':
                sisrloss_total = sisrloss + sisrloss2.mean()
            else:
            ...
            minval, minidx = l2diff.min(dim=1)
            sisrloss2 = torch.abs(SR_result - hrbatch)
            #selectionloss = torch.mean(-1 * probs.gather(1, minidx.unsqueeze(1)).clamp(1e-16).log() * l2diff.gather(1,minidx.unsqueeze(1)).detach())
            selectionloss = -1 * torch.mean(probs.gather(1, minidx.unsqueeze(1)).clamp(1e-16).log())
            agentloss_total = selectionloss
            agentloss_total.backward()
            self.agent.opt.step()

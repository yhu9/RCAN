...
class SISR():
    ...
    def mutualInformation(self,z,zt):
        ...
        P = ((P + P.t()) / 2) / P.sum()
        P = P.clamp(min=1e-16)
        Pi = P.sum(dim=1).view(self.k,1).expand(self.k,self.k)
        Pj = P.sum(dim=0).view(1,self.k).expand(self.k,self.k)
        return -1 * (P * (torch.log(P / (Pi * Pj)))).sum()
        #return -1 * (P * ((torch.log(Pi) + torch.log(Pj)) - torch.log(P))).sum()
    ...
    # train unsupervised image segmentation
    def optimizeSeg(self,data):
        while True:
            ...
            pred1 = self.agent.model1(lrbatch)
            pred1 = torch.nn.functional.softmax(pred1,dim=1)
            pred2 = self.agent.model2(lrbatch)
            pred2 = torch.nn.functional.softmax(pred2,dim=1)

            loss = self.mutualInformation(pred1.reshape(-1,self.k),pred2.reshape(-1,self.k))

            self.agent.opt1.zero_grad()
            self.agent.opt2.zero_grad()
            loss.backward()
            self.agent.opt1.step()
            self.agent.opt2.step()

            print(f"loss: {loss.item():.4f}")

            img_summaries = { 'sr/mask1': pred1[0][:3], 'sr/mask2': pred2[0][:3],'hr': hrbatch[0],'hr2': hrbatch[0]}

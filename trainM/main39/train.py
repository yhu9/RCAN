            ...
            for n,idx in enumerate(indices):
                ...
                for step in range(1):
                    ...
                    for j, sisr in enumerate(self.SRmodels):
                        ...
                        lr_pred = torch.nn.functional.interpolate(hr_pred,scale_factor=1/self.upsize,mode='bicubic')
                    ...
                    sisrloss_total = lossfn(hr_pred,hrbatch) + lossfn(lr_pred,lrbatch)
                    ...
                    img_summaries = {'sr/sr': SR_result[0].clamp(0,1),'sr/diffmap': diffmap[0][:3]}
                    ...
                    if self.logger.step % 100 == 0:
                        ...
                        if self.logger:
                            ...
                            self.logger.image_summary({'Testing/SR':srimg, 'Testing/HR': hrimg})
                        self.savemodels()
                    self.logger.incstep()

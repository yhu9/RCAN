            ...
            for n,idx in enumerate(indices):
                ...
                for step in range(1):
                    ...
                    if self.model == 'ESRGAN':
                        lrbatch = lrbatch * 1/255.0
                        hrbatch = hrbatch * 1/255.0
                    ...
                    if self.model == 'ESRGAN':
                        ...
                        l1diff = torch.mean(torch.abs(SR_result - hrbatch) / 255.0)
                    total_loss = l1diff
                    total_loss.backward()
                    ...
                    print('\rEpoch/img: {}/{} | Agent Loss: {:.4f}, SISR Loss: {:.4f}, c1: {:.4f}, c2: {:.4f}, c3: {:.4f} c4:{:.4f}'\
                            .format(c,n,total_loss.item(),loss_SISR, c1.item(), c2.item(), c3.item(),c4.item()),end="\n")
                    ...
                    if self.model == 'RCAN':
                        img_summaries = {'sr/mask': probs[0][:3], 'sr/sr': (SR_result[0]/255.0).clamp(0,1), 'sr/hr': (hrbatch[0]/255.0).clamp(0,1)}
                    else:

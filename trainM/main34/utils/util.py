...
def recombine(X,h1,w1,h2,w2):
    ...
    dh,dw = patchsize // 8, patchsize // 8
    #dh,dw = patchsize // 2, patchsize // 2
    #stride = patchsize//2
    padtop = (patchsize - dh) // 2
    padleft = (patchsize - dw) // 2
    padbot = -1 * padtop
    padright = -1*padleft
    ...
    img = torch.nn.functional.fold(tmp,(h2,w2),(patchsize,patchsize),1,0,(dh,dw))
    top = (patchsize - dh) // 2
    left = (patchsize - dw) // 2
    bot = h2 - h1 - top
    right = w2 - w1 - left

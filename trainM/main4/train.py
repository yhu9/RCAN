...
import utility
import model
from option import args as rcan_args
from importlib import import_module
from utils import util
from test import Tester

troch.manual_seed(args.seed)
########################################################################################################
...
parser.add_argument("--model_dir",default="",help='specify if restarting training')
parser.add_argument("--model",default="rcan",help='rcan,esrgan,random')
        ...
        if args.model_dir != "":
            ...
            for i in range(self.SR_COUNT):
                if args.model == 'esrgan':
                    ...
                    model = module.make_model(rcan_args).to(self.device)
                    checkpoint = utility.checkpoint(rcan_args)
                    model = model.Model(rcan_args,checkpoint)
